...
`public/morphs/ATTRIBUTION.md`에 20개 모프·패턴 대표 사진의 개별 출처와 권리 상태가 있습니다. 서로 다른 실제 개체 사진을 1600×1120 WebP 규격으로 맞춰 탐색 카드에만 사용합니다. 현재는 비공개 로컬 MVP 검토용이며, 공개 배포 전에 원 저작권자의 허가·라이선스를 확인하거나 소유 이미지로 교체해야 합니다. 대표 사진은 개별 판매 매물을 대신하지 않습니다. 실제 매물 카드는 원문이 공개한 썸네일만 사용하며, 썸네일이 없거나 불러오지 못하면 `이미지 없음`을 표시합니다.
