...
## 2026-08-14 — Reuse the home representative in the detail hero
- Context: the home card used the newly selected mid-market listing image, but the linked detail page still opened with an older curated category photo. This made one category look like two different animals and, for keyword groups, could weaken the visible connection to the selected market example.
- Decision: resolve the detail hero from the matching `getHomeMarketSnapshot()` summary by its canonical morph/trait href and pass that exact listing image URL to the shared detail page. Center listing-derived photos in both surfaces. If the URL is absent or fails, fall back client-side to the category's curated image and configured crop. Do not change the detail minimum, comparison population, or individual listing thumbnails.
- Consequences: home→detail navigation preserves the same representative animal for every inventory-backed morph and keyword, including overlapping keyword groups whose images were globally deduplicated on home. The detail hero gains a small client image boundary solely for failure recovery; the rest of the page remains server-rendered.
