...
- The home catalog derives categories from every registered morph and filterable keyword, omits groups with zero comparable current listings, sorts by current listing count descending, and does not duplicate the catalog with a promotional hero or global listing totals. Arbitrary seller-title tokens never become categories without dictionary review.
